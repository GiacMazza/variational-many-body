MODULE GZ_imtDYNAMICS  
  ...
  public :: gz_imt_eom
  public :: gz_imt_equations_of_motion_
  !
...
CONTAINS
  ...
  subroutine init_imt_qpH(beta0,gzproj,lgrNC_,tmp_Hqp)
    real(8) :: beta0
  ...
  subroutine gz_imt_measure_(psi_t,itime,slater_out,gzproj_out,Hqp_out)
    complex(8),dimension(nDynamics) :: psi_t
    real(8)                         :: itime
    complex(8),dimension(Ns,Ns,Lk),optional  :: slater_out
    ...
    gz_imt_energies(3) = Eloc
    !  
  end subroutine gz_imt_measure_
  !
  subroutine gz_imt_measure_constr_(psi_t,itime)
    complex(8),dimension(nDynamics) :: psi_t
    real(8)                         :: itime
    ...
    ! !
  end subroutine gz_imt_measure_constr_
  ...
  subroutine step_imt_dynamics(nsys,tstep,t,yt,itd_lgrNC,itd_lgrU,eom_funct) 
    ...
    lgrNC_new = itd_lgrNC    
    !lgrNC_new = lgrNC_old
    write(600,'(10F18.10)') t,gz_imt_qpH(490,1,1),gz_imt_qpH(490,1,1)
    call update_qpH_integral(yt_old,yt_new,lgrNC_old,lgrNC_new)
    write(601,'(10F18.10)') t,gz_imt_qpH(490,1,1),gz_imt_qpH(490,1,1)
    !    
    !
  contains
  ...
  subroutine step_imt_dynamics_(nsys,tstep,t,yt,itd_lgrNC,itd_lgrU,eom_funct) 
    integer                                   :: nsys
    real(8)                                   :: tstep,t
    complex(8),dimension(nsys),intent(inout)  :: yt
    complex(8),dimension(nsys)                :: yt_old,yt_new
  ...
  contains
    ...
    function imt_fix_constr(lgr) result(delta)
      ...
      !imt_lgrU = 0.d0
      imt_lgrU = lgr(2*Nvdm_NC_opt+1)
      !
      yt_new = RK4_step(nDynamics,4,itstep,t,yt_old,eom_funct)
      !
      call gz_imt_measure_constr_(yt_new,t) 
      !
      do is=1,Ns
         do js=1,Ns
            call get_imt_dens_constr_slater(is,js,normal_constrSL(is,js))
      ...
      deallocate(delta_cmplx)
      delta(2*Nvdm_NC_opt+1) = unitary_constrGZ-1.d0
      delta(2*Nvdm_NC_opt+2) = 0.d0
      !      
      ! tmp_test=0.d0
      ! do i=1,size(lgr)
      !    tmp_test=tmp_test+delta(i)**2.d0
  ...
  end subroutine step_imt_dynamics_
