function BCS_equations_of_motion(time,y,Nsys) result(f)
  ...
  delta_t = Uloc(1)*delta_t
  !
  do ik=1,Lk
     call get_Hk_t(Hk,ik,time)
     ekt = Hk(1,1)
     !
     bcsWF_dot(1,ik) = -2.d0*ekt*bcsWF(2,ik) - dimag(delta_t)*bcsWF(3,ik)
     bcsWF_dot(2,ik) =  2.d0*ekt*bcsWF(1,ik) - dreal(delta_t)*bcsWF(3,ik)
     bcsWF_dot(3,ik) =  dreal(delta_t)*bcsWF(2,ik) + dimag(delta_t)*bcsWF(1,ik)
     !
  end do
  bcsWF = -xi*bcsWF
  !
