program GUTZ_mb
  ...
  if(seed_file) then
     ...
     write(*,*) 'reading root seed from file Rn0_root_seed.conf'
...
CONTAINS
  ...
  subroutine print_output(vdm_simplex)
    ...
    complex(8),dimension(Ns,Ns) :: slater_vdm
    ...
    do is=1,Ns
       do js=1,Ns
          ...
          do ik=1,Lk
             ...
             slater_vdm(is,js) = slater_vdm(is,js) + GZ_opt_slater(is,js,ik)*wtk(ik)
          end do
          close(out_unit)
       end do
    end do
    ...
    do is=1,Ns
       do js=1,Ns
          ...
          write(out_unit,'(2F18.10)') dreal(GZ_opt_slater_lgr(is,js)),dimag(GZ_opt_slater_lgr(is,js))
          close(out_unit)
       end do
    end do
    do is=1,Ns
       do js=1,Ns
          ...
          write(out_unit,'(2F18.10)') dreal(GZ_opt_proj_lgr(is,js)),dimag(GZ_opt_proj_lgr(is,js))
          close(out_unit)
       end do
    end do
    !
    ...
    do istate=1,Ns
       ...
       write(out_unit,'(20F18.10)') dreal(GZ_opt_VDM(istate,:)),dimag(GZ_opt_VDM(istate,:))
    end do
...
!       jorb=iorb
    ...
    do iorb=1,Norb
       do jorb=iorb,Norb
          ...
          do ispin=1,2
             is=index(ispin,iorb)
             js=index(ispin,jorb)
             vdm_NC_mat(is,js) = vdm_NC_indep(iind)
             vdm_NC_mat(js,is) = vdm_NC_indep(iind)
             ! if(iorb.eq.1) then
             !    vdm_NC_mat(is,is) = vdm_NC_indep(1)
             ! else
             !    vdm_NC_mat(is,is) = 0.d0!vdm_NC_indep(2)
             ! end if
          end do
       end do
    end do
  ...
  subroutine vdm_NC_mat2vec(vdm_NC_mat,vdm_NC_indep)
    ...
    do iorb=1,Norb
       do jorb=iorb,Norb
          ...
          is=index(ispin,iorb)
          js=index(ispin,jorb)
          vdm_NC_indep(iind) = vdm_NC_mat(is,js)
          ! if(iorb.eq.1) then
  ...
  subroutine vdm_NCoff_mat2vec(vdm_NC_mat,vdm_NC_indep)
    ...
    is=index(ispin,iorb)
    js=index(ispin,jorb)
    vdm_NC_indep(1) = vdm_NC_mat(is,js)
    iorb=1
    jorb=3
    is=index(ispin,iorb)
    js=index(ispin,jorb)
    vdm_NC_indep(2) = vdm_NC_mat(is,js)
    iorb=2
    jorb=3
    is=index(ispin,iorb)
    js=index(ispin,jorb)
    vdm_NC_indep(3) = vdm_NC_mat(is,js)
    !
