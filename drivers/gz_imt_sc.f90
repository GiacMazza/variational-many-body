program GUTZ_mb
  ...
  complex(8),dimension(:,:,:,:),allocatable :: slater_init,Hqp_in
  ...
  !stop
  call imt_wfMatrix_superc_2_dynamicalVector(Hqp_in,gz_proj_init,psi_t)
  !
  eLevels = 0.d0
  ! !
  call get_local_hamiltonian_trace(eLevels)      
  !
  call setup_imt_dynamics_superc
  !    
  unit_imt_Rhop = free_unit()
  open(unit_imt_Rhop,file='imt_Rhop_matrix.data')
  !
  unit_imt_Qhop = free_unit()
  open(unit_imt_Qhop,file='imt_Qhop_matrix.data')  
  !
  ...
  !
  unit_imt_dens_constrSLa = free_unit()
  open(unit_imt_dens_constrSLa,file='imt_dens_constrSLa.data')
  !
  unit_imt_dens_constrGZa = free_unit()
  open(unit_imt_dens_constrGZa,file='imt_dens_constrGZa.data')
  !
  ...
  do im_it=1,Nit
     ...
     if(mod(im_it-1,nprint).eq.0) then        
        !
        call gz_imt_measure_superc(psi_t,t)
        !
        do is=1,Ns
           do js=1,Ns
              call get_imt_Rhop(is,js,Rhop_matrix(is,js))              
              call get_imt_Qhop(is,js,Qhop_matrix(is,js))              
