program GUTZ_mb
  ...
  integer,allocatable,dimension(:) :: ik2ix,ik2ieps
  real(8),allocatable,dimension(:) :: eps
  real(8) :: ts
  !
  real(8) :: t
  integer :: Nx,Neps,out_unit,is,js,ik,it,itt,i,iorb,ispin
  integer :: nprint
  !
  ...
  call parse_input_variable(Neps,"Neps","inputGZ.conf",default=2000)
  ...
  call parse_input_variable(tdLGR,"tdLGR","inputGZ.conf",default=.false.)  
  !
  ...
  call set_efield_vector(t_grid_aux)
  ...
  !
  !+- setup the non equilibrium hamiltonians -+!  
     ...
     if(mod(it-1,nprint).eq.0) then        
        ...
        if(bcs_neq) then
           ...
           do ik=1,Lk
              ...
              bcs_Kenergy = bcs_Kenergy + bcs_wf(3,ik)*epsik(ik)*wtk(ik)
           end do
           ...
           !
           psi_bcs_t = RK4_step(3*Lk,4,tstep,t,psi_bcs_t,bcs_equations_of_motion)
        end if
        !
     end if
     !
...
CONTAINS
  ...
  subroutine build_lattice_model  
    !
    integer :: ix,iy,iz,ik,Nk,ieps,istate,jstate,iorb,jorb
    real(8)                            :: test_k,kx_,ky_,kz_,wini,wfin,de,test_n1,test_n2,Wbethe
    !
    allocate(kx(Nx),eps(Neps))
    Lk=Nx*Neps

    Wbethe=2.d0/3.d0*Wband
    ...
    wini=-Wbethe*0.5d0
    wfin= Wbethe*0.5d0
    eps = linspace(wini,wfin,Neps,mesh=de)
    allocate(epsik(Lk),wtk(Lk),hybik(Lk))
    allocate(ik2ix(Lk),ik2ieps(Lk))
    ik=0
    ...
    do ix=1,Nx
       ...
       do ieps=1,Neps
          ik=ik+1
          epsik(ik) = eps(ieps) - 2.d0*ts*dcos(kx(ix))
          ik2ix(ik) = ix
          ik2ieps(ik) = ieps
          hybik(ik) = 0.d0
          if(eps(ieps).gt.-Wbethe/2.d0.and.eps(ieps).lt.Wbethe/2.d0) then
             wtk(ik)=4.d0/Wbethe/pi*sqrt(1.d0-(2.d0*eps(ieps)/Wbethe)**2.d0)*de
          else
             wtk(ik) = 0.d0
          end if
          wtk(ik) = wtk(ik)/dble(Nx)

          test_k=test_k + wtk(ik)
          write(77,*) epsik(ik),wtk(ik)
    ...
    write(*,*) 'testK',test_k
    !
    call get_free_dos(epsik,wtk,file='DOS_free.kgrid',wmin=-5d0,wmax=5d0)
    !
  ...
  subroutine getHk(Hk,ik,time)
    ...
    !
    Ak = Afield(time)
    ekt = eps(ieps) - 2.d0*ts*dcos(kx(ix)-Ak(1))
    !ekt = -2.d0/6.d0*(cos(kx(ix)-Ak(1))+cos(kx(iy)-Ak(2))+cos(kx(iz)-Ak(3)))
    !
    Hk=zero
    do is=1,Ns
       Hk(is,is) = ekt
