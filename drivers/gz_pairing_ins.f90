program GUTZ_mb
  ...
  !+- simple solution of the variational problem
  ...
  ! open(xmu_unit,file='optimized.out')
  ! do i=1,Nsweep
  !    ! write(dir_suffix,'(F6.2)') Uiter
  !    ! dir_suffix=adjustl(dir_suffix)
  !    ! dir_iter="XMU"//trim(dir_suffix)

  !    ! call system('mkdir -v '//dir_iter)     
  !    xmu=Uiter
  !    call get_gz_optimized_density_VS_xmu_min_simple(xmu,Zhop,Docc,xdop,ene) 

  !    write(xmu_unit,*) xmu,Zhop,Docc,xdop,ene
  ...
  do i=1,Nsweep
     ...
     if(nread/=0.d0) then
        ...
        call bracket_density_min_simple(xmu1,xmu2,0.01d0,300)     
        write(xmu_unit,*)  xmu1,xmu2
        close(xmu_unit)
        !
        xmu_unit=free_unit()
        open(xmu_unit,file='search_XMU.out') 
        search_mu=zbrent(gz_optimized_density_VS_xmu_min_simple,xmu1,xmu2,nerr)       
        close(xmu_unit)     
        !
        call get_gz_optimized_density_VS_xmu_min_simple(search_mu,Zhop,Docc,xdop,ene) 
        !        
     else
        !
        search_mu=0.d0
     ...
     xmu_unit=free_unit()
     open(xmu_unit,file='optimized.out')
     write(xmu_unit,*) Zhop,Docc,xdop,ene,xmu
     close(xmu_unit)

     !
     call system('cp * '//dir_iter)
     call system('rm *.out *.data  ')
     Uiter = Uiter + sweep_step
  end do
  !
CONTAINS
  ...
  subroutine bracket_density_min_simple(xmu1,xmu2,xmu_step,xmu_loop)
    real(8),intent(inout) :: xmu1,xmu2
    real(8)               :: xmu_step
    real(8)               :: dens1,dens2,xmu,dens,sign_dens,xmu_max,xmu_min
    real(8),dimension(2)  :: xmu_sort
    ...
    dens1 = gz_optimized_density_VS_xmu_min_simple(xmu1)
    dens2 = gz_optimized_density_VS_xmu_min_simple(xmu2)
    ...
    if(sign_dens.gt.0.d0) then
       ...
       if(dens1.gt.0.d0) then          
          ...
          do i=1,xmu_loop
             xmu = xmu - abs(xmu_step)
             dens = gz_optimized_density_VS_xmu_min_simple(xmu)
             sign_dens = dens*dens1
             write(*,*) 'loop bracket',xmu,xmu_sort,dens,dens1,dens2,sign_dens
             if(sign_dens.lt.0.d0) then
                ! take the largest between xmu1 and xmu2 and <--- xmu
    ...
    xmu2=xmu_sort(2)
    write(*,*) 'BRACKET OUT',xmu1,xmu2,dens,dens1,dens2
  end subroutine bracket_density_min_simple
  ...
  function gz_optimized_density_VS_xmu_min_simple(chem_pot) result(delta_local_density)
    real(8),intent(in)   :: chem_pot
    real(8)              :: delta_local_density
    real(8),allocatable,dimension(:,:) :: p
    real(8),allocatable,dimension(:)   :: y
    real(8)                            :: ftol
    integer                            :: np,mp,i_vertex,j_vertex,i_dim,iter
    real(8) :: docc,tangth,xdop
    !                                                                                                                                                                                                           
    xmu = chem_pot
    ! NP=2
    ! MP=NP+1
    ...
    ! !+- initialize simplex -+!                                                                                                                        
    ! p(1,1)=0.00;p(1,2)=0.1
    ! p(2,1)=0.10;p(2,2)=0.1
    ! p(3,1)=0.30;p(3,2)=0.2
    ! !
    ! do i_vertex=1,MP
    !    y(i_vertex) = ene_min(p(i_vertex,:))
    ! end do
    ! !
    ! ftol=1.d-12
    ! call amoeba(p(1:MP,1:NP),y(1:MP),ftol,ene_min,iter,amoeba_verbose)
    ! stop
    ! docc = p(1,2)
    ! xdop = 2.d0*docc*dsin(p(1,1))
    ! !xdop = p(1,1)
    ! delta_local_density = 1-xdop-Nread
    
    ! docc=p(1,1)
    ! tangth=-4.d0*xmu/(1.d0-2.d0*docc)
    ! if(xmu.ge.0.d0) then
    !    delta_local_density = -2.d0*docc*sqrt(tangth**2.d0/(tangth**2.d0+1.d0))
    ! else
    !    delta_local_density = 2.d0*docc*sqrt(tangth**2.d0/(tangth**2.d0+1.d0))
    ! end if
    ! delta_local_density = 1-delta_local_density - Nread
    !write(*,*) delta_local_density
    !write(xmu_unit,*) xmu,delta_local_density,docc,Nread
    ...
    !
    do i_vertex=1,MP
       y(i_vertex) = dene_min(p(i_vertex,:))
    end do
    !
    ftol=1.d-12
    call amoeba(p(1:MP,1:NP),y(1:MP),ftol,dene_min,iter,amoeba_verbose)
    ...
    !
  end function gz_optimized_density_VS_xmu_min_simple
  !


  subroutine get_gz_optimized_density_VS_xmu_min_simple(chem_pot,Z,docc,xdop,ene) 
    real(8),intent(in)   :: chem_pot
    real(8)              :: delta_local_density
    real(8),allocatable,dimension(:,:) :: p
    real(8),allocatable,dimension(:)   :: y
    ...
    integer                            :: np,mp,i_vertex,j_vertex,i_dim,iter
    !      
    real(8) :: Z,docc,tangth,xdop,ene
    !
    xmu = chem_pot!;write(*,*) xmu,chem_pot
    ...
    tangth=-4.d0*xmu/(1.d0-2.d0*docc)
    if(xmu.ge.0.d0) then
       xdop = -2.d0*docc*sqrt(tangth**2.d0/(tangth**2.d0+1.d0))
    else
       xdop = 2.d0*docc*sqrt(tangth**2.d0/(tangth**2.d0+1.d0))
    end if        
    Z = 4.0*(1-2*docc)/(1-xdop*xdop)*(docc+sqrt(docc**2-xdop**2/4))
    ene=y(1)
    ...
    write(111,*)
    !
  end subroutine get_gz_optimized_density_VS_xmu_min_simple



  function ene_min(x) result(GZenergy)
    real(8),dimension(:),intent(in) :: x
    real(8)               :: GZenergy
    real(8) :: d,dop,th,n,dmax

    th=x(1);d=x(2)
    dop=2*d*dsin(th)
    n=1.d0-dop
    !dmax=0.5d0-dop*0.5d0!;write(*,*) dmax;stop
    dmax=0.5d0
    if(d.le.dmax.and.d.gt.0) then
       GZenergy = -0.5*(1.d0-2*d)*(d+sqrt(d*d-dop*dop/4)) + Uloc(1)*d - xmu*(1.-dop)
    else
       GZenergy=100.d0
    end if
    write(111,*) d,GZenergy,dop,xmu,n,-0.5*(1.d0-2*d)*(d+sqrt(d*d-dop*dop/4)),Uloc(1)*d,- xmu*(1.-dop)

  end function ene_min
  !

  function dene_min(x) result(GZenergy)
    real(8),dimension(:),intent(in) :: x
    real(8)               :: GZenergy
    real(8) :: d,dop,th,n,dmax,sgn,dmin,costh,sinth
    ...
    if(nread/=0.d0) then
       ...
       dmax=0.5d0
       dmin=(1.d0-nread)*0.5d0
    end if
    sinth=(1.0-Nread)/2.d0*d
    if(d.le.dmax.and.d.ge.dmin) then
       !GZenergy=-0.5d0*d*(1.d0-2*d)*(1+(1-2*d)/sqrt((1-2*d)**2+16*xmu**2)) + Uloc(1)*d - 8*xmu**2*d/sqrt((1-2*d)**2+16*xmu**2)
       GZenergy=-0.5d0*d*(1.d0+costh)*(1-2*d) + Uloc(1)*d !- xmu*(1-2*d*sinth)
       if(d.eq.0.5d0.and.xmu.eq.0.d0) then
          GZenergy=Uloc(1)*d
          !write(*,*) 'here',d,GZenergy !
       end if
    else
       GZenergy=100.d0
    end if
    ! write(*,*) d,GZenergy
    write(111,*) d,GZenergy

  end function dene_min
