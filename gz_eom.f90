...
function gz_equations_of_motion_slater_superc_lgr(time,y,Nsys) result(f)
  ...
  if(Nsys.ne.nDynamics) stop "wrong dimensions in the GZ_equations_of_motion"
  !
  call dynamicalSlater_2_wfMatrix_superc(y,slater_)
  slater(1:2,:,:,:) = slater_(1:2,:,:,:)
  slater(3,:,:,:) = zero
  do is=1,Ns
     do js=1,Ns
  ...
  !
  t0=t_grid(1)
  it = t2it(time,tstep*0.5d0,t0)
  Rhop = neq_Rhop_ext(it,:,:)
  Qhop = neq_Qhop_ext(it,:,:)
  do is=1,Ns
     do js=1,Ns
        Rhop_hc(is,js) = conjg(Rhop(js,is))
        Qhop_hc(is,js) = conjg(Qhop(js,is))
  ...
  slater_dot = -xi*slater_dot  
  ! 
  call wfMatrix_superc_2_dynamicalSlater(slater_dot,f)
  !
end function gz_equations_of_motion_slater_superc_lgr
...
contains
  ...
  !
end function gz_eom_superc_lgr_sp_fsolveSL
...
contains
  ...
  subroutine get_SLa_not(y,anomalous_constrSL_dot,constrSL)
    ...
    do ik=1,Lk
       ...
       !
       ! Hk_full(1:Ns,1:Ns) = tRR
       ! tmp(1:Ns,1:Ns) = slater(1,:,:,ik)
       ! !
       ! Hk_full(1:Ns,1+Ns:2*Ns) = tRQ
       ! tmp(1:Ns,1+Ns:2*Ns) = slater(2,:,:,ik)
       ! !
       ! Hk_full(1+Ns:2*Ns,1:Ns) = tQR
       ! tmp(1+Ns:2*Ns,1:Ns) = conjg(transpose(slater(2,:,:,ik)))
       ! !
       ! Hk_full(1+Ns:2*Ns,1+Ns:2*Ns) = tQQ
       ! tmp(1+Ns:2*Ns,1+Ns:2*Ns) = slater(3,:,:,ik)
       ! !
       ! dotK=matmul(tmp,transpose(Hk_full)) - matmul(transpose(Hk_full),tmp)

       ! anomalous_constrSL_dot = anomalous_constrSL_dot + dotK(1:Ns,1+Ns:2*Ns)*wtk(ik)
...
function gz_eom_slater_superc_lgr(time,y,Nsys) result(f)
  ...
  call fsolve(fix_anomalous_vdm_sl,lgr,tol=10.d-12,info=iter)
  delta_out = fix_anomalous_vdm_sl(lgr);
  write(*,*) "SL constr fixed",delta_out
  write(*,*) "SL lgr ",lgr
  lgr_cmplx=zero
  do i=1,Nvdm_AC_opt
     lgr_cmplx(i) = lgr(i)+xi*lgr(i+Nvdm_AC_opt)
  ...
  call vdm_AC_stride_v2m(lgr_cmplx,neq_lgr(1,:,:))
  gz_neq_dens_lgrA_slater = neq_lgr(1,:,:)
  !write(*,*) gz_neq_dens_lgrA_slater
  !
  f=zero
  !f = gz_equations_of_motion_slater_superc_lgr(time,y,Nsys)
  !
contains
  ...
  !
end function gz_eom_slater_superc_lgr
