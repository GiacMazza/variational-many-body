...
function R_VDM_free_zeros_(x)  result(Fout)
  ...
  !
  call slater_minimization_fixed_lgr_(Rhop,slater_lgr_multip,E_Hstar,n0_slater,slater_derivatives)  
  !
  do is=1,Ns
     do js=1,Ns
        Rhop_lgr_multip(is,js) = -1.d0*slater_derivatives(is,js)/sqrt(n0_slater(js,js)*(1.d0-n0_slater(js,js)))
  ...
  !
  call gz_proj_minimization_fixed_Rhop(n0_diag,proj_lgr_multip,Rhop_lgr_multip,E_Hloc,GZvect)    
  !
  n0_GZproj = 0.d0
  do is=1,Ns
     do js=1,Ns
  ...
  ! write(*,*) "DEBUG_N0"
  ! do is=1,Ns
  !    write(*,'(10F8.4)') slater_out(is,:)
  ! end do
  ! write(*,*)
  ! do is=1,Ns
  !    write(*,'(10F8.4)') n0_GZproj(is,:)
  ! end do
  !DEBUG>
  !
