...
function R_VDM_free_zeros_new(x)  result(Fout)   !+- change this name
  real(8),dimension(:)                :: x      
  !
  real(8),dimension(size(x))          :: Fout
  real(8),dimension(size(x))          :: tmp_x
  ...
  complex(8),dimension(Ns,Ns)       :: slater_derivatives
  complex(8),dimension(Ns,Ns)       :: slater_lgr_multip
  complex(8),dimension(Ns,Ns)       :: slater_lgr_offD
  complex(8),dimension(Ns,Ns)       :: proj_lgr_multip
  complex(8),dimension(Ns,Ns)         :: Rhop_lgr_multip
  !
  complex(8),dimension(Ns,Ns)       :: n0_slater,n0_GZproj
  ...
  !
  complex(8),dimension(Ns,Ns)         :: Rhop_out
  complex(8),dimension(Ns,Ns)       :: NC_out
  logical                           :: test_n
  !
  NRhop = 2*NRhop_opt
  !
  ...
  if(size(x).ne.Nopt) stop "R_VDM_free_zeros_new: size input vector not equal to Nopt."
  !
  call dump2mats_(x,Rhop,slater_lgr_multip)
  !  
  !


  call slater_minimization_fix_offDiag(Rhop,slater_lgr_multip,slater_lgr_offD,E_Hstar,n0_slater,slater_derivatives,iverbose=GZmin_verbose)  
  !
  test_n=.true.
  !  
  do is=1,Ns
  ...
  write(*,*) "n0_slater"
  do is=1,Ns
     write(*,'(20F18.10)') n0_slater(is,:)
  end do
  write(*,*)
  do is=1,Ns
     write(*,'(20F18.10)') slater_derivatives(is,:)
  end do
  !stop
  !TMP>
  ...
  do is=1,Ns
     ...
     do js=1,Ns
        tmp = tmp + dreal(Rhop_lgr_multip(is,js)*Rhop(is,js))
        proj_lgr_multip(is,js) = 0.5d0*slater_lgr_multip(is,js)
     end do
     proj_lgr_multip(is,is) = proj_lgr_multip(is,is) + &  
          0.5d0*(1.d0-2.d0*n0_slater(is,is))/sqrt(n0_slater(is,is)*(1.d0-n0_slater(is,is)))*tmp 
  end do
  ...
  write(*,*) "lgr gz"
  do is=1,Ns
     write(*,'(20F18.10)') proj_lgr_multip(is,:)
  end do
  !TMP>
  ...
  do is=1,Ns
     do js=1,Ns
        Rhop_out(is,js) = Rhop_GZproj(is,js)-Rhop(is,js)*sqrt(n0_diag(js)*(1.d0-n0_diag(js)))
        NC_out(is,js) = n0_GZproj(is,js) -  n0_slater(is,js)
     end do
  end do
  !
  ! do is=1,Ns
  ...
  ! write(*,*)
  ! do is=1,Ns
  !    write(*,*) dreal(NC_out(is,:))
  !    write(*,*) imag(NC_out(is,:))
  ! end do
  ! ! do is=1,Ns
  ! !    write(*,*) dreal(GZproj_out(is,:))
  ! !    write(*,*) imag(GZproj_out(is,:))
  ...
  call dump2vec_(tmp_x,Rhop_out,NC_out)
  !
  Fout=tmp_x
  delta_out=0.d0
  do is=1,Nopt
  ...
  if(optimization_flag) then
     ...
     GZ_opt_Eloc    = E_Hloc
     !+- here I should modify this ?? porca troia quante robbe da modificare...
     GZ_opt_slater_lgr = slater_lgr_multip + slater_lgr_offD
     GZ_opt_proj_lgr = proj_lgr_multip
     GZ_opt_Rhop = Rhop
     !
     ...
     do is=1,Ns
        do js=1,Ns
           write(*,*) trace_phi_basis(GZ_vector,phi_traces_basis_dens(is,js,:,:))
        end do
     end do

  end if
  !
end function R_VDM_free_zeros_new
!
...
function R_VDM_free_zeros__new(x)  result(Fout)   !+- change this name
  real(8),dimension(:)                :: x      
  real(8),dimension(size(x))          :: Fout
  !
  integer                             :: Nslater_lgr,NQhop,NRhop,Nproj_lgr,Nopt
  ...
  allocate(x_orig(Nopt),Fout_orig(Nopt))
  call stride_zeros_red2orig(x,x_orig)
  Fout_orig = R_VDM_free_zeros_new(x_orig)
  call stride_zeros_orig2red(Fout_orig,Fout)
  !
end function R_VDM_free_zeros__new
...
subroutine dump2vec_(x,Rhop,slater_lgr)
  real(8),dimension(:)                :: x
  complex(8),dimension(Ns,Ns)         :: Rhop
  complex(8),dimension(Ns,Ns)       :: slater_lgr
  !complex(8),dimension(Ns,Ns)       :: GZproj_lgr
  ...
  ! end if
  !
end subroutine dump2vec_
!
subroutine dump2mats_(x,Rhop,slater_lgr_multip)
  real(8),dimension(:)                :: x
  complex(8),dimension(Ns,Ns)         :: Rhop
  complex(8),dimension(Ns,Ns)       :: slater_lgr_multip
  !complex(8),dimension(Ns,Ns)       :: proj_lgr_multip
  ...
  Nopt = 2*NRhop_opt + 2*Nvdm_NC_opt !+ 2*Nvdm_NCoff_opt 
  write(*,*) size(x),Nopt
  if(size(x).ne.Nopt) stop "wrong dimensions @ dump2mats_"
  !
  Rhop = zero
  slater_lgr_multip = zero
  !proj_lgr_multip = zero
