subroutine build_two_sites_fock_algebra
  integer                            :: fock_state,ifock
  integer                            :: i,iup,jup,idw,iup_,idw_,np,nup_
  integer                            :: ispin,iorb,istate,isite
  ...
  do ispin=1,2
     do iorb=1,Norb
        do isite=1,Nsite
           istate=i_ios(ispin,iorb,isite)
           call build_c_operator(ispin,iorb,isite,CA(istate,:,:))
           call build_cdg_operator(ispin,iorb,isite,CC(istate,:,:))          
        end do
     end do
  end do
  !
contains
  !
  subroutine build_c_operator(ispin,iorb,isite,Cmat)
    integer                        :: ispin,iorb,isite
    real(8),dimension(nFock,nFock) :: Cmat
    integer                        :: imp,i,j
    integer                        :: ib(Ns)
    real(8)                        :: c_
    !build <j|C|i>
    imp = i_ios(ispin,iorb,isite)
    Cmat=0d0
    do i=1,nFock
       call bdecomp(i,ib)
       if(ib(imp)==0)cycle
  ...
  !
  subroutine build_cdg_operator(ispin,iorb,isite,Cmat)
    integer                        :: ispin,iorb,isite
    real(8),dimension(nFock,nFock) :: Cmat
    integer                        :: imp,i,j
    integer                        :: ib(Ns)
...
end subroutine build_two_sites_fock_algebra
